...
#	* Adjusting the Pokedex to accomidate Touhoumon changes
#	* Makes it so it displays Yin/Yang for Puppets in the Pokedex
#	* Makes it display Cost for Puppets in the Pokedex
#	* Makes it display "[Kind] Puppet" for Puppets
#	* Made the height and weight display in Metric for accuracy to the games
#==============================================================================#
class PokemonPokedexInfo_Scene
  def pbStartScene(dexlist, index, region)
    ...
    @typebitmap = AnimatedBitmap.new(_INTL("Graphics/UI/Pokedex/icon_types_big")) # Derx: Changed
    @sprites = {}
  ...
  def pbGetAvailableForms
    ...
    ret.each do |entry|
      if entry[0]   # Alternate forms, and form 0 if no gender differences
        ...
        case entry[1]
		when 0 then entry[0] = species_data.has_flag?("Puppet") ? _INTL("Yin") : _INTL("Male")
		when 1 then entry[0] = species_data.has_flag?("Puppet") ? _INTL("Yang") : _INTL("Female")
        else
  ...
  def drawPageInfo
    ...
    if @show_battled_count
      ...
      if species_data.has_flag?("Puppet")
		textpos.push([_INTL("Cost"), 314, 196, :left, base, shadow])
      else
    ...
    if $player.owned?(@species)
      # Write the category
      if species_data.has_flag?("Puppet")
		textpos.push([_INTL("{1} Puppet", species_data.category), 246, 80, :left, base, shadow])
      else
      ...
      if !@show_battled_count
        ...
        if species_data.has_flag?("Puppet")
          ...
          textpos.push([_ISPRINTF("{1:.1f}  ", weight / 10.0), 482, 196, :right, base, shadow])
        else
      ...
      # Write the category
      if species_data.has_flag?("Puppet")
      	textpos.push([_INTL("????? Puppet"), 246, 80, :left, base, shadow])
      else
      ...
      if !@show_battled_count
          ...
          textpos.push([_INTL("????.?  "), 482, 196, :right, base, shadow])
        else
          if System.user_language[3..4] == "US"   # If the user is in the United States
