...
class PokemonPartyScreen
  def pbPokemonScreen
    ...
    @scene.pbStartScene(@party,
                        (@party.length > 1) ? _INTL("Choose a party member.") : _INTL("Choose a party member or cancel."),
                        nil, false, can_access_storage)
    loop do
      @scene.pbSetHelpText((@party.length > 1) ? _INTL("Choose a party member.") : _INTL("Choose a party member or cancel."))
