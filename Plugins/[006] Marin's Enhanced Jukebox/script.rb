...
class EnhancedJukebox
  ...
  def initialize
    ...
    if NOW_PLAYING_TEXT != ""
      @sprites["playingtext"].draw(
          [NOW_PLAYING_TEXT, 26, 342, 0, BASECOLOR, SHADOWCOLOR]
      )
  ...
  def update_song
    name = $game_temp.defaultBGM&.name || "Default Music"
    name = name.split("/")[-1]
    ...
    def draw_items
      ...
      for i in @top...(@top + EnhancedJukebox::ITEMS_DRAWN)
        ...
        if width >= @viewport.rect.width - x
          ...
          @sprites["text"].draw(
              [text, x, 10 + 32 * (i - @top), 0, EnhancedJukebox::BASECOLOR, EnhancedJukebox::SHADOWCOLOR]
          )
        end
      end
    end
...
class Game_System
  ...
  def setDefaultBGM(bgm, volume = 80, pitch = 100)
    ...
    if bgm && bgm.name != ""
      $game_temp.defaultBGM = bgm.clone
      self.bgm_play(bgm)
    else
      $game_temp.defaultBGM = nil
  ...
  def bgm_stop # :nodoc:
    ...
    @playing_bgm  = nil
    Audio.bgm_stop if !$game_temp.defaultBGM
  end

  def bgm_fade(time) # :nodoc:
    @bgm_position = 0 if !@bgm_paused
    @playing_bgm = nil
    Audio.bgm_fade((time*1000).floor) if !$game_temp.defaultBGM
  end
end
